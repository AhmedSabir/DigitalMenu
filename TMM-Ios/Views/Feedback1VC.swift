...
//  TMM-Ios
//
//  Created by Jigar Joshi on 12/14/18.
...
class Feedback1VC: UIViewController,UITableViewDelegate,UITableViewDataSource,UITextFieldDelegate,PassAttributeData,PasstextData
{
    ...
    override func viewDidLoad() {
        ...
        ImageLoader_SDWebImage.getImage(themeData[0].popupBackground) { (image, error) in
            if let img = image {
    ...
    override func viewDidLayoutSubviews() {
      //  self.contblFeedbackHeight.constant = self.view.frame.size.height - 750
        contblFeedbackHeight.constant = CGFloat((arrFeedBack!.count * 100))
        self.tblFeedBack.updateConstraintsIfNeeded()
    ...
    func downloadThemes(ThemeID: Int32){
        ...
        SkopelosClient.shared.read { context in
            ...
            ImageLoader_SDWebImage.getImage(self.themeData[0].background) { (image, error) in
                if let img = image {
    ...
    @IBAction func btnSubmitClick(_ sender: UIButton)
    {
        ...
        else
        {
            ...
            OperationQueue.main.addOperation({
                ...
                if self.strIsCome == "fini"
                {
                    let objVC: FiniHomeVC? = self.storyboard?.instantiateViewController(withIdentifier: "FiniHomeVC") as? FiniHomeVC
...
//                else
//                {
//                    let objVC: PortraitModeVC? = self.storyboard?.instantiateViewController(withIdentifier: "PortraitModeVC") as? PortraitModeVC
//                    objVC?.strCome = "feedback"
//                    let aObjNavi = UINavigationController(rootViewController: objVC!)
//                    let appDelegate = UIApplication.shared.delegate as! AppDelegate
//                    appDelegate.window?.rootViewController = aObjNavi
...
//                }
                      ...
                      //strIsCome == "HomePageVC"
                      {
                          let objVC: HomePageVC? = self.storyboard?.instantiateViewController(withIdentifier: "HomePageVC") as? HomePageVC
                          //objVC?.strCome = "feedback"
                          let aObjNavi = UINavigationController(rootViewController: objVC!)
                          let appDelegate = UIApplication.shared.delegate as! AppDelegate
                          appDelegate.window?.rootViewController = aObjNavi
