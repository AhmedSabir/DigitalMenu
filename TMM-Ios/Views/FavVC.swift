...
class FavVC: UIViewController,UITableViewDelegate,UITableViewDataSource,UIGestureRecognizerDelegate
{
...
//        let strUrl = "http://37.34.173.73:865/mobile/userlogin?Password=222"
//        String(format: "http://%@:%@/mobile/userlogin?Password=865",getIp,String(format: "%d", getPort))
...
//                    verified = true
    ...
    func wscallForplaceOrder()
    {
        ...
        SkopelosClient.shared.read { context in
            ...
            {
                print(dict)
                let dicttemp = ["Comments" : arrcomments , "Extras" : arrcomments , "SendtoPrinter" : "false" , "TableNO" : self.txtTableNO.text! , "OrderItemID" : dict.orderItemID , "ItemID": dict.itemID as Any, "ItemName" : dict.itemName as Any , "ItemNameArabic" : dict.itemNameAr as Any , "Price" : dict.price as Any, "Quantity" : dict.qty as Any] as [String : AnyObject]
    ...
    
    {
        ...
        SkopelosClient.shared.read { context in
            ...
            ImageLoader_SDWebImage.getImage(self.themeData[0].categoryActive) { (image, _) in
                self.btnClearFav.setBackgroundImage((image ?? ImageLoader_SDWebImage.placeholder), for: [])
            }
            
            self.tblFav.backgroundColor = UIColor.clear
            ...
            ImageLoader_SDWebImage.getImage(self.themeData[0].categoryInactive) { (image, _) in
                self.btnClearcart.setBackgroundImage((image ?? ImageLoader_SDWebImage.placeholder), for: [])
                self.btnProcess.setBackgroundImage((image ?? ImageLoader_SDWebImage.placeholder), for: [])
                self.btnCancel.setBackgroundImage((image ?? ImageLoader_SDWebImage.placeholder), for: [])
            }
...
//                self.btnSendOrder.titleLabel?.textColor = UIColor.black
...
//                btnSendOrder.setTitleColor(color1, for: .normal)
                self.btnCancel.setTitleColor(color1, for: .normal)
                self.lblGrandTotalData.textColor  = color1
                self.lblGrandTotal.textColor  = color1

//                self.btnClearcart.titleLabel?.textColor  = color1
//                self.btnSendOrder.titleLabel?.textColor = color1
